# Auth Service

Простой и эффективный gRPC микросервис для аутентификации пользователей с поддержкой подтверждения email через RabbitMQ.

## 🚀 Возможности

- **Регистрация пользователей** с хэшированием паролей
- **Отправка email подтверждения** через RabbitMQ
- **gRPC API** для интеграции с другими сервисами
- **Метрики Prometheus** для мониторинга
- **Миграции базы данных** с помощью golang-migrate
```

## 🏗️ Архитектура

```
auth-service/
├── api/
│   └── proto/
│       └── api.proto              # gRPC контракты
├── internal/
│   ├── config/                    # Конфигурация приложения
│   │   ├── config.go
│   │   └── database.go
│   ├── domain/                    # Доменный слой
│   │   ├── entities/
│   │   │   └── user.go           # Сущность пользователя
│   │   ├── value_objects/
│   │   │   └── user_vo.go        # Value Objects
│   │   └── repositories/         # Интерфейсы репозиториев
│   │       ├── rabbit_repository.go
│   │       └── user_repository.go
│   ├── services/                 # Бизнес-логика
│   │   └── user_service.go
│   ├── infrastructure/           # Инфраструктурный слой
│   │   ├── http/
│   │   │   ├── handlers/
│   │   │   │   └── user_handler.go
│   │   └── implementations/
│   │       ├── postgres/         # Реализация PostgreSQL
│   │       │   └── user_repository.go
│   │       └── broker/           # Реализация RabbitMQ
│   │           └── rabbit_repository.go
│   ├── middleware/               # gRPC middleware
│   │   ├── logger.go
│   │   └── metrics.go
│   ├── monitoring/               # Мониторинг и метрики
│   │   ├── metrics.go
│   │   └── server.go
│   └── utils/
│       ├── hashing/
│       │   └── password.go       # Хэширование паролей
│       ├── service_errors/
│       │   └── errors.go         # Бизнес-ошибки
│       └── strings.go            # Вспомогательные функции
├── migrations/                   # Миграции базы данных
│   └── 0001_init_migrations.up.sql
├── cmd/
│   └── server/
│       └── main.go               # Точка входа приложения
├── .env.example                  # Пример переменных окружения
├── prometheus.yml               # Конфигурация Prometheus
└── go.mod
```

## 📊 Схема базы данных

```sql
CREATE TABLE users (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    email VARCHAR(255) NOT NULL UNIQUE,
    password BYTEA NOT NULL,
    is_active BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
```

## 🛠️ Технологии

- **gRPC** - высокопроизводительный RPC фреймворк
- **PostgreSQL** - реляционная база данных
- **RabbitMQ** - брокер сообщений для асинхронной обработки
- **Squirrel** - SQL builder для построения запросов
- **validator-playground** - валидация данных
- **bcrypt** - хэширование паролей
- **Prometheus** - сбор метрик и мониторинг
- **golang-migrate** - управление миграциями БД

## ⚡ Быстрый старт

### 1. Клонирование и настройка

```bash
git clone <repository-url>
cd auth-service
cp .env.example .env
```

### 2. Генерация gRPC кода (если нужно изменить proto)

```bash
protoc --go_out=. --go-grpc_out=. api/proto/api.proto
```

### 3. Запуск зависимостей

```bash
# PostgreSQL
docker run -d -p 5432:5432 -e POSTGRES_PASSWORD=password postgres:15

# RabbitMQ
docker run -d -p 5672:5672 -p 15672:15672 rabbitmq:3-management

# Prometheus
docker run -d -p 9090:9090 prom/prometheus:latest
```

### 4. Запуск приложения

```bash
go run cmd/server/main.go
```

## 🔧 Конфигурация

Создайте `.env` файл на основе `.env.example`:

```env
DB_HOST=localhost
DB_PORT=5432
DB_USER=postgres
DB_PASSWORD=your-password
DB_NAME=your-db

RABBIT_HOST=localhost
RABBIT_PORT=5672
RABBIT_LOGIN=guest
RABBIT_PASSWORD=guest

ACCESS_TOKEN_EXPIRE_MINUTES=30
REFRESH_TOKEN_EXPIRE_DAYS=10
SECRET_KEY=your-secret-key
ALGORITHM=yur-algorithm

METRICS_PORT=2112
```

## 📡 API

### gRPC методы

```proto
service AuthService {
  rpc Register(AuthRequest) returns (RegisterResponse);
  rpc Login(AuthRequest) returns (AuthResponse);
  rpc RefreshTokens(RefreshToken) returns (AuthResponse);
}
```

### Метрики

Приложение предоставляет метрики по адресу:

```
http://localhost:2112/metrics
```

## 🔄 Поток регистрации

1. **Запрос регистрации** через gRPC
2. **Проверка уникальности** email в базе данных
3. **Хэширование пароля** с помощью bcrypt
4. **Создание пользователя** в PostgreSQL
5. **Отправка сообщения** в RabbitMQ для подтверждения email
6. **Возврат результата** клиенту

## 📈 Мониторинг

Сервис предоставляет метрики для Prometheus:

- `grpc_requests_total` - количество gRPC запросов
- `grpc_request_duration_milliseconds` - время выполнения запросов
- `grpc_requests_in_flight` - текущие выполняемые запросы

## 🔒 Безопасность

- Пароли хэшируются с помощью bcrypt
- Валидация входных данных
- Обработка ошибок без утечки чувствительной информации

## 🤝 Разработка

### Локальная разработка

```bash
# Запуск приложения в режиме разработки
go run cmd/server/main.go
```

---

**Примечание**: Этот сервис предназначен для использования в составе микросервисной архитектуры и требует дополнительной
настройки для production использования.

```